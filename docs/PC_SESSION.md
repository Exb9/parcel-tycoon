...
5. `ServerStorage.CosmeticTemplates.trailRainbow` (Trail).
...
`StarterGui.HUD`, `StarterGui.Windows` (sieben Fenster) und `StarterGui.Toasts` mit genau den Namen aus
der Elementliste, feste Texte mit `TextKey`. Prüfen: Play drücken, keine Zeile mit `[UI]` im Output.
...
- [ ] Daily und Quests: abholen; `daily` macht die Tagesbelohnung wieder abholbar, `quests done`
      erledigt die Quests.
- [ ] Store: in Studio zeigen Angebote „Test“ und werden direkt gewährt (Pässe, Cash, Boost, Instant
      Machine, Styles). Style-Fenster: Trail, Neon-Bänder, glänzende Pakete, Party-Halle an/aus.
- [ ] VIP: ohne Pass blockt die Tür mit Hinweis, `pass vip` → Tür lässt durch.
- [ ] Offline-Einnahmen: `offline 3600` zahlt eine Stunde aus.
...
- Gamepässe und Developer Products im Creator Hub anlegen, Preise dort setzen und die IDs in
  `src/shared/Config/Monetization.luau` eintragen (0 = noch nicht angelegt).
- Werden Developer Products auch im Store-Tab der Spielseite verkauft, dort nur Cash-Pakete und den Boost
  anbieten (Styles und Instant Machine sind im Spiel an Bedingungen geknüpft).
